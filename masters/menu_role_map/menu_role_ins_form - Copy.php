...
function print_tree($i,$par_nod,$par_mod,$par_rol,$con){
   $qry="SELECT * FROM `n_mast_menu` WHERE `pnmid` = '$i';";
    ...
    if($cnt==0){
        ...
        foreach($a_par as $a_row1){
          $v_pnmid1=$a_row1['pnmid'];
          $qry1="SELECT * FROM `n_mast_menu` WHERE `pnmid` = '$v_pnmid1' AND `modid`='$par_mod';";
          $a_par1=mysqli_query($con, $qry1);
          foreach($a_par1 as $a_row){
          ...
          echo '<tr style="border-top:1pt solid black;'.$v_dispr.'"> 
                <td style="padding-left:'.$i.'0pt;border-right:1pt solid black;">
                  <input type="text" name="menid[]" value="'.$v_tmenid.'">
